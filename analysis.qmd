...
ggplot(top_cities_2020, aes(x = population, y = cinema_audiences_num, label = city)) +
  ...
  theme_minimal() +
  labs(title = "2020 Yılında Sinema Seyirci Sayısı En Yüksek 10 Şehir",
       x = "Nufus",
       y = "Sinema Seyirci Sayısı") +
...
data_2020 <- data_2020 %>%
  mutate(loaned_book_per_capita = loaned_book_num / population)
...
ggplot(top_literacy_cities_2020, aes(x = loaned_book_per_capita, y = suicides_num, color = city)) +
  geom_point() +
  geom_text_repel(aes(label = city)) +
  labs(x = "Loaned Book Number per Capita", y = "Number of Suicides",
...
ggplot(top_literacy_cities, aes(x = loaned_book_num, y = suicides_num, colour = city)) +
  ...
  theme_minimal() +
  theme(legend.position = "bottom")  # Efsaneyi altta göstermek için
=======
```{r}
#| code-fold: true
#| code-summary: "Show the code"
...
>>>>>>> d6b1aa65e8fbaaebbf3649436cf216491ef458eb
